 ...
 *
 * Returns the pathname the browser landed on after a successful login, so
 * the caller can continue the EXISTING same-origin `crawlApplication` BFS
 * from there. Returns undefined — a safe no-op, never a guess — when the
 * page doesn't look like a login form, no credential was supplied, or the
 * login attempt itself fails; callers must fall back to whatever start
 * path/storageState they were already configured with.
 */
